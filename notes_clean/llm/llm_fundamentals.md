...
🎯 **Interview Triggers:**
- "Self-Attention 为什么要除以 √d_k？"（WHY）
- "Transformer 和 RNN 相比有什么根本优势和劣势？"（TRADEOFF）
- "为什么 Transformer 推理成本随序列长度平方增长？"（FAILURE）

🧠 **Question Type:** principle explanation · comparison/tradeoff · performance optimization

🔥 **Follow-up Paths:**
- 注意力计算复杂度 → KV Cache 优化
- 多头注意力 → 不同 head 学到不同特征
- 位置编码 → RoPE/ALiBi

🛠 **Engineering Hooks:**
- 序列长度超 4K 时注意 O(n²) 显存爆炸
- Flash Attention 降低显存到 O(n)
- 监控 attention pattern 可视化诊断模型行为
- d_model / num_heads 必须整除
...
🎯 **Interview Triggers:**
- "BPE 和 WordPiece 有什么区别？"（TRADEOFF）
- "为什么中文 token 效率比英文低？"（WHY）
- "tokenizer 不一致会导致什么问题？"（FAILURE）

🧠 **Question Type:** principle explanation · comparison/tradeoff · debugging/failure analysis

🔥 **Follow-up Paths:**
- vocab_size 选择 → 影响 Embedding 层参数量
- tokenizer 版本锁定 → 推理部署一致性
- 特殊 token → system/user/assistant 格式

🛠 **Engineering Hooks:**
- tokenizer 版本必须和模型版本锁定一致（常见部署 bug 来源）
- 中文约 1.5~2 字/token，英文约 0.75 词/token
- vocab_size 通常 32K~128K
- 特殊 token（bos/eos/pad）在推理时必须正确设置
...
🎯 **Interview Triggers:**
- "什么情况下用 LoRA 而不是全参数微调？"（TRADEOFF）
- "SFT 之后为什么还需要 RLHF？"（WHY）
- "微调后模型在通用任务上退化，怎么解决？"（FAILURE）

🧠 **Question Type:** comparison/tradeoff · system design · debugging/failure analysis

🔥 **Follow-up Paths:**
- LoRA rank 选择 → 精度 vs 显存 tradeoff
- RLHF vs DPO → 对齐方法复杂度
- 灾难性遗忘 → 数据配比和正则化

🛠 **Engineering Hooks:**
- LoRA `r=16~64`（r=16 通常足够，r=64 接近全参数）
- `lora_alpha=2r`（常用经验值）
- SFT 数据量门槛 500~1000 条高质量对
- 监控验证集 loss 和通用 benchmark 防止过拟合
...
🎯 **Interview Triggers:**
- "Temperature=0 和 Greedy Decoding 的区别？"（WHY）
- "Top-p 和 Top-k 哪个更常用，为什么？"（TRADEOFF）
- "什么情况下模型会陷入重复循环？"（FAILURE）

🧠 **Question Type:** principle explanation · comparison/tradeoff · debugging/failure analysis

🔥 **Follow-up Paths:**
- Temperature → 控制输出多样性 vs 确定性
- Beam Search → 机器翻译场景仍常用
- 重复惩罚 → `repetition_penalty` 参数

🛠 **Engineering Hooks:**
- 创意任务 `temperature=0.7~1.0`；代码/事实任务 `temperature=0~0.2`
- 生产默认用 Top-p（`p=0.9`）而非 Top-k
- 出现重复循环加 `repetition_penalty=1.1~1.3`
- 结构化输出场景用 `temperature=0` + JSON mode
...
🎯 **Interview Triggers:**
- "KV Cache 为什么能加速推理？存了什么？"（WHY）
- "KV Cache 显存占用和哪些因素成正比？"（TRADEOFF）
- "KV Cache 在什么情况下失效或不起作用？"（FAILURE）

🧠 **Question Type:** principle explanation · performance optimization · debugging/failure analysis

🔥 **Follow-up Paths:**
- KV Cache 显存 → PagedAttention 优化（vLLM）
- Prefix Caching → 固定 system prompt 的 KV 复用
- 多并发请求 → KV Cache 不共享的内存浪费

🛠 **Engineering Hooks:**
- KV Cache 显存 ≈ 2 × num_layers × num_heads × head_dim × seq_len × batch_size × dtype_bytes
- 开启 Prefix Caching（共享 system prompt KV）可降低 30~50% 显存
- PagedAttention 将 KV Cache 碎片化为 pages，GPU 利用率从 30% → 90%
- 监控 KV Cache 命中率（vLLM metrics）
...
🎯 **Interview Triggers:**
- "Chinchilla 法则对训练有什么实践意义？"（WHY）
- "大模型 scaling 的瓶颈在哪里？"（TRADEOFF）
- "为什么有些大模型效果不如小模型？"（FAILURE）

🧠 **Question Type:** principle explanation · comparison/tradeoff · system design

🔥 **Follow-up Paths:**
- 参数量 vs Token 量 → Chinchilla 最优配比
- 模型压缩 → 量化/蒸馏让小模型接近大模型
- 推理成本 → 参数量决定推理延迟和显存

🛠 **Engineering Hooks:**
- Chinchilla 最优：tokens ≈ 20 × parameters（如 70B 模型需 1.4T tokens）
- 监控训练 loss 曲线，loss 不再下降时考虑增加数据而非参数
- 推理显存估算：参数量（GB）≈ params × dtype_bytes（FP16=2B，INT8=1B）
- 小模型+更多数据通常优于大模型+少数据
...
🎯 **Interview Triggers:**
- "RoPE 相对于绝对位置编码的核心优势是什么？"（WHY）
- "RoPE 和 ALiBi 在长序列外推能力上有什么差异？"（TRADEOFF）
- "模型处理超出训练长度的序列时会发生什么？"（FAILURE）

🧠 **Question Type:** principle explanation · comparison/tradeoff · performance optimization

🔥 **Follow-up Paths:**
- 长上下文扩展 → RoPE Scaling（YaRN/LongRoPE）
- 位置编码与 Attention → 影响远距离 token 注意力权重
- 外推失败 → 困惑度暴增的诊断

🛠 **Engineering Hooks:**
- RoPE 是当前主流（LLaMA/Qwen/Mistral 均用）
- 长上下文扩展用 `rope_scaling={"type": "yarn", "factor": 4.0}`
- ALiBi 不需要额外参数，适合长度变化大的任务
- 超出训练长度时监控困惑度（perplexity）是否暴增
...
🎯 **Interview Triggers:**
- "开源模型和闭源模型的核心差距在哪，什么场景必须用闭源？"（TRADEOFF）
- "为什么同参数量的模型效果差距很大？"（WHY）
- "在生产部署中选错模型会导致什么问题？"（FAILURE）

🧠 **Question Type:** comparison/tradeoff · system design

🔥 **Follow-up Paths:**
- 模型选型 → llm_engineering_practice.md Model Routing
- 量化部署 → 小参数模型 + INT4 量化可在消费级 GPU 运行
- 模型评估 → Chatbot Arena ELO vs 业务 eval set

🛠 **Engineering Hooks:**
- 生产选型优先看 Chatbot Arena ELO + 业务 eval set，不看单一 benchmark
- 开源部署显存估算：7B FP16≈14GB，7B INT4≈4GB
- API 成本对比：Claude Haiku ~$0.25/1M tokens vs Claude Opus ~$75/1M tokens
- 定期用新版模型跑 A/B 测试，每季度评估是否需要升级
