...
🎯 **Interview Triggers:**
- 量化为什么能减少显存，原理是什么？（WHY）
- INT8 和 INT4 量化分别损失多少精度，什么时候不能用 INT4？（TRADEOFF）
- 量化后模型在哪些任务上最容易退化？（FAILURE）
...
🛠 **Engineering Hooks:**
- INT8 精度损失 <1%（安全）
- INT4 损失 1~3%（推理可接受，微调不推荐）
- GPTQ 离线量化 + 存文件；AWQ 保护重要权重效果更好
- llama.cpp 用 GGUF 格式支持 CPU 推理
- 监控量化前后的 perplexity 差值
...
🎯 **Interview Triggers:**
- 为什么 LLM 推理 GPU 利用率只有 30%，Continuous Batching 怎么解决？（WHY）
- Static Batching 和 Continuous Batching 的核心区别在哪？（TRADEOFF）
- Continuous Batching 在什么场景下效果不明显？（FAILURE）

🧠 **Question Type:** principle explanation · performance optimization · comparison/tradeoff

🔥 **Follow-up Paths:**
- GPU 利用率 → PagedAttention 进一步优化显存
- 吞吐量 vs 延迟 tradeoff → 批大小调整
- 推理框架 → vLLM 实现细节

🛠 **Engineering Hooks:**
- 监控 GPU utilization（目标 >80%，Static Batching 通常 30%）
- `max_batch_size` 调优：显存允许范围内尽量大
- 监控请求队列长度和 P99 延迟
- 长序列请求会 block 短序列 → 设置优先级队列或序列长度限制
...
🎯 **Interview Triggers:**
- KV Cache 显存碎片化是什么问题，PagedAttention 怎么解决？（WHY）
- PagedAttention 对延迟有影响吗？（TRADEOFF）
- KV Cache 显存不够时会发生什么？（FAILURE）

🧠 **Question Type:** principle explanation · performance optimization · system design

🔥 **Follow-up Paths:**
- KV Cache 显存 → 影响最大并发请求数
- Prefix Sharing → 相同 system prompt 共享 KV Block
- vLLM 架构 → 推理框架选型

🛠 **Engineering Hooks:**
- PagedAttention block_size 通常 16 tokens
- 监控 KV Cache 命中率（vLLM `/metrics`）
- Prefix Caching 开启：`enable_prefix_caching=True`（固定 system prompt 场景节省 30~50% 显存）
- OOM 时先减 `max_model_len` 再减 `gpu_memory_utilization`
...
🎯 **Interview Triggers:**
- 多 GPU 推理时通信开销在哪里，怎么最小化？（WHY）
- 张量并行和流水线并行各自适合什么场景？（TRADEOFF）
- 推理并行配置错误会导致什么问题？（FAILURE）

🧠 **Question Type:** comparison/tradeoff · system design · performance optimization

🔥 **Follow-up Paths:**
- 张量并行 → GPU 间通信带宽要求（NVLink vs PCIe）
- 流水线并行 → 跨节点部署
- 量化 + 并行组合 → 超大模型部署方案

🛠 **Engineering Hooks:**
- 同机多卡优先用张量并行（TP，低通信延迟）；跨节点用流水线并行（PP）
- TP 要求 GPU 间 NVLink，PCIe 通信瓶颈显著
- `tensor_parallel_size` 必须整除 `num_attention_heads`
- 监控 GPU 间通信带宽利用率
...
🎯 **Interview Triggers:**
- 为什么不直接用 HuggingFace Transformers 做生产推理？（WHY）
- vLLM 和 TensorRT-LLM 的核心差距在哪，什么情况选哪个？（TRADEOFF）
- 推理框架选型错误有哪些常见代价？（FAILURE）

🧠 **Question Type:** comparison/tradeoff · system design

🔥 **Follow-up Paths:**
- vLLM → PagedAttention + Continuous Batching 细节
- TRT-LLM → NVIDIA 专用优化栈
- llama.cpp → 边缘部署；SGLang → 复杂 LLM 程序高效执行

🛠 **Engineering Hooks:**
- 快速原型/云端 API → vLLM（安装简单，社区活跃）
- 最高吞吐量 NVIDIA 硬件 → TensorRT-LLM（需编译，复杂）
- 离线/边缘/CPU → llama.cpp（GGUF 格式，零依赖）
- 监控 tokens/second、P50/P95 TTFT、GPU Memory 使用率
...
🎯 **Interview Triggers:**
- 投机解码为什么能加速推理，数学上是否保证等价？（WHY）
- 草稿模型接受率低时会怎样，有什么代价？（FAILURE）
- 投机解码和 Continuous Batching 可以结合吗？（TRADEOFF）

🧠 **Question Type:** principle explanation · performance optimization · comparison/tradeoff

🔥 **Follow-up Paths:**
- 草稿模型选型 → 参数量比例（通常 1:10）影响接受率
- Medusa → 同模型多 head 投机，无需额外模型
- 批处理兼容性 → 投机解码在高并发时收益下降

🛠 **Engineering Hooks:**
- 草稿模型接受率目标 >70%（低于此加速收益不明显）
- 草稿模型大小通常是目标模型 1/10（如 7B draft → 70B target）
- Medusa 头数 `medusa_num_heads=4~5`
- 投机解码在低并发（batch=1）场景收益最大，高并发时收益下降
- vLLM 支持 `speculative_model` 参数直接配置
...
🎯 **Interview Triggers:**
- 用户感知到的延迟由哪些部分组成，最应该优化哪一段？（WHY）
- TTFT 和 TPOT 分别受什么因素影响？（TRADEOFF）
- 推理延迟突然增大，应该从哪里开始排查？（FAILURE）

🧠 **Question Type:** performance optimization · debugging/failure analysis · principle explanation

🔥 **Follow-up Paths:**
- TTFT 优化 → Prefill 并行化
- TPOT 优化 → Decode 阶段 KV Cache 命中
- 端到端监控 → LangSmith / Prometheus + Grafana

🛠 **Engineering Hooks:**
- 监控 TTFT P50/P95（目标 <2s）和 TPOT P50（目标 <50ms/token）
- Prefill 阶段受 input_tokens 长度影响最大，压缩 prompt 可显著降低 TTFT
- Decode 阶段受 KV Cache 命中率影响
- 延迟暴增排查顺序：GPU 显存 → KV Cache OOM → batch 积压 → 网络 I/O
