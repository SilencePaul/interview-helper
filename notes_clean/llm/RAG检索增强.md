...
🎯 **Interview Triggers:**
- "RAG 解决了 LLM 的什么根本缺陷，为什么不直接扩大训练数据？"（WHY）
- "你们的 RAG 系统在哪些场景下效果不好，怎么排查？"（FAILURE）
- "Naive RAG 和 Advanced RAG 的核心差距在哪，升级代价是什么？"（TRADEOFF）

🧠 **Question Type:** system design · debugging/failure analysis · comparison/tradeoff

🔥 **Follow-up Paths:**
- 检索质量不准 → 混合检索（BM25 + Dense + RRF）
- 生成阶段幻觉 → Faithfulness 指标 + Prompt 约束
- 端到端评估 → RAGAS 四大指标
- 是否需要 Fine-tuning → RAG vs Fine-tuning 决策框架

🛠 **Engineering Hooks:**
- `top_k=3~5`：生产起点，用 RAGAS Context Precision 量化后再调
- 检索延迟分段监控：向量检索 / Re-rank / LLM 各自计时（LangSmith span）
- 向量库选型：Pinecone（全托管）/ Milvus（自托管）/ pgvector（已有 PG 时）
- 文档更新策略：增量 upsert（小更新）vs 全量重建（大版本）
- RAGAS 定期跑：每次改动检索策略或 Prompt 后触发评估 CI
...
🎯 **Interview Triggers:**
- "Chunk Size 过大和过小分别导致什么问题？有没有黄金参数？"（TRADEOFF）
- "代码文件、表格、Markdown 各应该用什么分块策略，为什么？"（WHY）
- "父子分块解决了什么矛盾，代价是什么？"（TRADEOFF）

🧠 **Question Type:** principle explanation · performance optimization · comparison/tradeoff

🔥 **Follow-up Paths:**
- 分块质量 → 直接影响 Embedding 向量的语义完整性
- 父子分块 → Small-to-Big 和 Context Recall 指标的关系
- 特殊内容处理 → 表格/代码/图片的多模态分块策略

🛠 **Engineering Hooks:**
- `chunk_size=256~512`，`overlap=50~100 token`（约 10~20%）作为初始参数
- 工具：`RecursiveCharacterTextSplitter`（通用）/ `MarkdownHeaderSplitter`（结构文档）
- 代码文件：按函数/类边界切，不用字符数切割
- 监控块大小分布：绘制直方图，检测异常过大或过小的 chunk
- 用 Context Recall 指标验证分块质量，召回不全时先减小 chunk_size 或增大 Top-K
...
🎯 **Interview Triggers:**
- "为什么用 ANN 而不是精确 KNN？精度损失能接受吗？"（TRADEOFF）
- "Bi-encoder 和 Cross-encoder 为什么分两个阶段使用，合并成一个会怎样？"（WHY）
- "通用 Embedding 在专业领域效果差，怎么解决？"（FAILURE）

🧠 **Question Type:** comparison/tradeoff · principle explanation · performance optimization

🔥 **Follow-up Paths:**
- ANN 召回率不足 → HNSW 参数调优（ef_search、M）
- Embedding 模型效果差 → 领域微调（Contrastive Learning）
- 单路向量检索不够 → 混合检索（加 BM25 补关键词）

🛠 **Engineering Hooks:**
- HNSW 参数：`ef_construction`（构建精度）、`M`（图连接数）、`ef_search`（查询精度），通常 `M=16, ef=200` 起步
- 相似度 cutoff：设最低阈值（如 `score > 0.7`），过滤低相关结果
- 模型选型：BGE-M3（开源多语言）/ text-embedding-3-large（闭源强效）/ domain fine-tuned（专业领域）
- 监控检索 p95 延迟：ANN 正常 <10ms，超出说明索引或硬件需优化
- 定期离线评估 Recall@K：用标注集检验 ANN 是否漏掉了精确匹配
...
🎯 **Interview Triggers:**
- "纯向量检索在什么场景下明显失效？"（FAILURE）
- "RRF 公式为什么不需要调权重，背后的直觉是什么？"（WHY）
- "Re-ranking 每次都需要做吗，延迟和精度怎么权衡？"（TRADEOFF）

🧠 **Question Type:** comparison/tradeoff · performance optimization · principle explanation

🔥 **Follow-up Paths:**
- 混合检索后仍然不准 → 查询优化（HyDE / Multi-query）
- 精排效果量化 → Context Precision 指标
- 延迟敏感场景 → FlashRank / ColBERT 轻量重排方案

🛠 **Engineering Hooks:**
- BM25 + Dense 融合：RRF `k=60`（默认值，通常无需调整）
- Re-ranker 输入：召回 Top-50，精排后取 Top-5（可调参数）
- Re-ranker 选型：BGE-reranker-v2（开源）/ Cohere Rerank（闭源 API）/ FlashRank（低延迟）
- 监控 Context Precision：RAGAS 定量衡量精排效果，Precision 低则加强 Re-rank
- 延迟预算：混合检索 + Re-rank 通常增加 100~200ms，超出时换轻量 Re-ranker
...
🎯 **Interview Triggers:**
- "HyDE 的核心假设是什么，什么情况下会适得其反？"（FAILURE/WHY）
- "Multi-query 和 Sub-query 适用场景有何不同，代价各是什么？"（TRADEOFF）
- "你们的 RAG 召回质量很差，有哪些可能原因，你会怎么排查？"（FAILURE）

🧠 **Question Type:** principle explanation · comparison/tradeoff · debugging/failure analysis

🔥 **Follow-up Paths:**
- HyDE 假答案质量 → 上游 LLM 幻觉对检索方向的传导影响
- 多轮对话 → 历史 query 如何融合到当前检索（Context-aware Query）
- 延迟预算 → 各优化方法 LLM 调用次数对比，如何做取舍

🛠 **Engineering Hooks:**
- HyDE 温度：`temperature=0.7~1.0` 生成假答案，高温增多样性但增幻觉风险
- Multi-query 数量：生成 3~5 个变体，结果去重后合并送检索
- 延迟成本：每种查询优化 = 额外 1 次 LLM 调用（约 +300~500ms），按需开启
- A/B 对比：固定 eval set，对比加/不加查询优化前后的 Context Recall 变化
- 仅在 Context Recall 指标低时引入，避免对简单 query 过度优化
...
🎯 **Interview Triggers:**
- "企业内部知识库应该用 RAG 还是 Fine-tuning，怎么判断？"（TRADEOFF）
- "Fine-tuning 之后为什么还需要 RAG，两者不是重叠的吗？"（WHY）
- "什么情况下 RAG 明显不如 Fine-tuning，为什么？"（FAILURE）

🧠 **Question Type:** comparison/tradeoff · system design

🔥 **Follow-up Paths:**
- Fine-tuning 方法选型 → LoRA / QLoRA（参数量 vs 效果 tradeoff）
- 两者组合架构 → SFT 定行为 + RAG 补知识的具体工程设计
- 成本分析 → 训练 GPU 时间 vs 向量库运维成本的长期对比

🛠 **Engineering Hooks:**
- 决策顺序：先跑 RAGAS 指标验证 RAG 基线，再决定是否 Fine-tune
- Fine-tuning 数据量门槛：SFT 最少 500~1000 条高质量对话对
- LoRA rank：`r=16~64`，越大越接近全参数微调，通常 r=16 足够
- 组合部署：Fine-tuned 模型 + 独立向量数据库，RAG pipeline 结构不变
- 监控 Faithfulness 回归：Fine-tune 后 Faithfulness 可能下降，需重新校准
...
🎯 **Interview Triggers:**
- "没有 Ground Truth 标注，如何快速判断 RAG 系统好不好？"（WHY）
- "Faithfulness 低应该优化检索还是生成，怎么区分？"（FAILURE）
- "RAGAS 各指标低分分别指向什么根本问题，修复路径是什么？"（debugging）

🧠 **Question Type:** debugging/failure analysis · performance optimization · principle explanation

🔥 **Follow-up Paths:**
- Faithfulness 持续低 → 区分是检索带入噪声还是 Prompt 约束不足
- Context Recall 低 → 返回混合检索 / Top-K 调整
- LLM-as-Judge 机制 → 深入了解评估模型的偏差（evaluation.md）

🛠 **Engineering Hooks:**
- 评估触发时机：每次改动 Prompt、检索策略或 Re-ranker 后自动触发 RAGAS CI
- 最小样本量：≥100 条，覆盖边缘案例（长文档、多跳问题、无答案问题）
- 裁判模型选型：GPT-4o（最准）/ Claude Sonnet（成本均衡）
- 生产达标线参考：`Faithfulness > 0.8`，`Answer Relevancy > 0.8`
- 人工校验：每月随机抽 20~30 条对比 RAGAS 判断和人工判断，检测评估漂移
